...
from storage import allocate_device_id_range, initialize_storage, release_shard_lease  # noqa: E402
...
def on_test_stop(environment: Any, **kwargs: Any) -> None:
    """Release the shard lease when the test stops."""
    logger.info("Test stopping, releasing shard lease")
    release_shard_lease()
...
# Connection jitter: maximum random delay (in seconds) before each device connects
# This spreads out connection attempts to avoid thundering herd on test start
# Set to 0 to disable jitter
connection_jitter_max = float(os.getenv("CONNECTION_JITTER_MAX", "240.0"))
...
class CertUserThrottle(User):
    ...
    Environment Variables:
        ...
        HUB_INDEX: Index of the hub to use (default: 0)
    ...
    def _ensure_id_range_unlocked(cls) -> None:
        ...
        This method allocates a range from Azure Blob Storage only once per worker.
        If no range has been allocated yet, it will allocate one.
        ...
        """
        if not cls._id_range_allocated:
            logger.info(f"Allocating device ID range for prefix '{device_name_prefix}'")
            cls._id_range_start, cls._id_range_end = allocate_device_id_range(
                device_name_prefix, storage_only_mode=use_stored_devices_only, hub_index=hub_index
            )
    ...
    @classmethod
    def get_device_name(cls) -> str | None:
        ...
        1. Allocating non-overlapping ID ranges from Azure Blob Storage (once per worker)
        ...
        Returns:
            A unique device name in the format "{prefix}{id}", or None if range exhausted.
        """
        with cls._id_range_lock:
            cls._ensure_id_range_unlocked()
            if cls._id_range_current >= cls._id_range_end:
                logger.warning(f"Device ID range exhausted: allocated [{cls._id_range_start}, {cls._id_range_end})")
                return None
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        for i in range(devices_per_user):
            ...
            if device_name is None:
                logger.info(f"ID range exhausted, created {len(self.devices)} device(s) for this user")
                break
    ...
    def on_start(self) -> None:
        ...
        # Use gevent pool for parallel initialization within this user
        pool = Pool(size=min(len(self.devices), 10))
        pool.map(self._init_device, self.devices)
    ...
    def _init_device(self, device: HubCertDevice) -> None:
        """Initialize a single device (provision and connect)."""
        # Add random jitter to spread out connection attempts
        if connection_jitter_max > 0:
            jitter = random.uniform(0, connection_jitter_max)
            gevent.sleep(jitter)
        ...
        if not self._is_device_provisioned(device):
            if not device.provision(storage_only=use_stored_devices_only):
                if use_stored_devices_only:
                    ...
                    self.devices.remove(device)  # Remove from list to avoid future attempts
                else:
                    logger.warning(f"Failed to provision device {device.device_name} during startup")
                return  # provision() already handles connect on success
    ...
    def request_certificate(self) -> None:
        ...
        device.request_new_certificate(replace=True, use_invalid_replace=False)
